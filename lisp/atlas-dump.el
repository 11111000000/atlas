...
WHAT may be 'all or a list among (files symbols edges summaries inv).
FORMAT is 'sexp or 'jsonl. PATH is required."
  (unless path (user-error "atlas-dump: PATH is required"))
  (let* ((root (file-name-as-directory (expand-file-name root)))
         (what (if (eq what 'all) '(files symbols edges summaries inv) what))
    ...
    (pcase format
      ('sexp
       (with-temp-buffer
         (prin1 (list :files files :symbols symbols :edges edges :summaries sums :inv inv) (current-buffer))
         (insert "\n")
         (write-region (point-min) (point-max) path nil 'silent)))
      ('jsonl
       (with-temp-buffer
         (dolist (pair `((files . ,files) (symbols . ,symbols) (edges . ,edges) (summaries . ,sums) (inv . ,inv)))
...
BATCH keys: :files, :symbols, :edges, :summaries, optional :file for per-file replace."
  (let* ((root (file-name-as-directory (expand-file-name root)))
         (state (or (atlas-state root) (atlas-open root))))
