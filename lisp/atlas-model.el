...
TTL/expiry (when present) are enforced; expired facts are ignored.
Pinned facts (:pinned t) take precedence over others."
  (let* ((sb (atlas-model--get-idx state atlas-model--k-facts-by-subj))
         (res '())
       ...
       (let ((by (make-hash-table :test #'eq)))
         (dolist (f lst)
           (let* ((pred (plist-get f :predicate))
                  ...
                  (ttl  (plist-get f :ttl))
                  (exp  (plist-get f :expires))
                  (expired?
                   (or (and (numberp exp) (> now exp))
                       (and (numberp ttl) (numberp ts) (> now (+ ts ttl))))))
             (when (and (>= conf minc) (not expired?))
               (atlas-model--push by pred f))))
         (maphash
          (lambda (_pred flist)
            ;; pick best by (pinned, src-priority, conf, ts, weight)
            (let ((best nil))
              (dolist (f flist)
                (let* ((pinned (and (plist-get f :pinned) t))
                       (pin    (if pinned 1 0))
                       (src    (plist-get f :source))
                       (pri    (atlas-model--source-priority src))
                       (conf   (or (plist-get f :confidence) 0.0))
                       (ts     (or (plist-get f :ts) 0.0))
                       (w      (or (plist-get f :weight) 0.0)))
                  (when (or (null best)
                            (> pin (plist-get best :_pin))
                            (and (= pin (plist-get best :_pin))
                                 (or (> pri (plist-get best :_pri))
                                     (and (= pri (plist-get best :_pri))
                                          (or (> conf (plist-get best :_conf))
                                              (and (= conf (plist-get best :_conf))
                                                   (or (> ts (plist-get best :_ts))
                                                       (> w (plist-get best :_w)))))))))
                    (setq best (list :_pin pin :_pri pri :_conf conf :_ts ts :_w w :fact f)))))
